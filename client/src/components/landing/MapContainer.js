...
const MapContainer = (props) => {
  ...
  const getData = async () => {
    const response = await axios.get(`/api/caterers`)
    console.log('response = ', response.data)
   
        dispatch({ type: SET_CATERERS, caterers: response.data });
          ...
          (
            ...
            >
              <div className="infoWindow" style={{opacity: 1}}>
                <div className="message" style={{fontWeight:"bold", color:"red"}}>{selected.shop}</div>
                ...
                <span className="store-des"><PhoneIcon className="icon-ui"/>{selected.phone}</span>
              </div>
            </InfoWindow>
          )
        }
