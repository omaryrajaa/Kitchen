...
export default function Orders(props) {
  ...
  const handleRadioChange = (event, index) => {
    ...
    dispatch({
      type: SET_CATERER_ORDERS_TODAY,
      catererOrdersToday: updatedTodayOrder,
    });
  ...
  const updateData = (value, index) => {
    axios
      .put(
        `/api/caterers/${catererId}/orders/${state.catererOrdersToday[index].order}`,
        {
          orderId: state.catererOrdersToday[index].order,
  ...
  const handleOrderDetails = (event, index) => {
    event.preventDefault();
    if (catererId) {
      history.push(`/order-details/${state.catererOrdersToday[index].order}`, {
        params: {
          catererId,
          orderId: state.catererOrdersToday[index].order,
          amount: state.catererOrdersToday[index].amount,
        },
      });
    } else {
      history.push("/login");
  ...
  const getData = async () => {
    ...
    dispatch({
      type: SET_CATERER_ORDERS_TODAY,
      catererOrdersToday: response.data,
    });
        ...
        <TableBody>
          {state.catererOrdersToday.map((row, index) => (
            <TableRow key={row.order}>
              ...
              <TableCell>
                <Link onClick={(event) => handleOrderDetails(event, index)}>
                  {row.order}
                </Link>
              </TableCell>
